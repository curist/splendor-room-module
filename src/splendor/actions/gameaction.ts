...
B.on('gameaction/pick-card', (db, action) => {
  db.set(['game', 'action'], {
    action: 'pick-card',
    card: action.card,
  });
...
B.on('gameaction/take-resource', (db, action) => {
...
B.on('gameaction/hold-a-rank-card', (db, action) => {
  db.set(['game', 'action'], {
    action: 'blind-hold',
    rank: action.rank,
  });
...
function endTurn(db) {
  ...
  if(shouldDropResources(player)) {
    ...
    B.exec(db, { action: 'gameevent/drop-resource' });
    return;
  }
  // nobles check
...
B.on('gameaction/pick-noble', (db, action) => {
  const { noble } = action;
